...
export function OrgActivityLogPanel({ rows }: { rows: OrgActivityRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter(
      (r) => r.memberName.toLowerCase().includes(q) || (r.competency ?? "").toLowerCase().includes(q),
    );
  }, [rows, query]);
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {filtered.map((row) => {
                ...
                return (
                  <tr key={`${row.kind}-${row.id}`} className="border-b border-card-border last:border-0">
                    <td className="py-2.5 pr-4 font-medium text-foreground">{row.memberName}</td>
                    ...
                    <td className="py-2.5">
                      {row.detailHref ? (
                        <Link href={row.detailHref} className="text-sm font-medium text-accent hover:underline">
                          상세 보기
                        </Link>
                      ) : (
                        <span className="rounded-full bg-background px-2 py-0.5 text-xs text-muted">비공개</span>
                      )}
                    </td>
                  </tr>
                );
