...
export function OrgOpsConsole({
  ...
  cohort: CohortData;
  people?: OrgPeopleDashboardData | null;
  activityPreview: OrgActivityRow[];
  pendingCount?: number;
  initialTab?: OrgOpsTab;
}) {
  ...
  const [people, setPeople] = useState<OrgPeopleDashboardData | null>(initialPeople);
  ...
  // 구성원/개요 탭에서 SSR 데이터가 없으면 API로 지연 로드
  useEffect(() => {
    if (tab !== "people" && tab !== "overview") return;
    if (people) return;
    let cancelled = false;
    ...
    void fetch("/api/org/people")
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(json.error ?? "구성원 데이터를 불러오지 못했습니다.");
        if (!cancelled) setPeople(json as OrgPeopleDashboardData);
      })
      .catch((e) => {
        if (!cancelled) {
          setPeopleError(e instanceof Error ? e.message : "구성원 데이터를 불러오지 못했습니다.");
        }
      })
      .finally(() => {
              ...
              {tab === "overview" ? (
                people ? (
                  ...
                  <p className="text-sm text-muted">
                    {peopleLoading
                      ? "개요 지표를 불러오는 중…"
                      : peopleError ?? "개요 데이터를 불러올 수 없습니다."}
                  </p>
                )
              ) : null}
              {tab === "people" ? (
                people ? (
                  ...
                  <p className="text-sm text-muted">
                    {peopleLoading
                      ? "구성원 명단을 불러오는 중…"
                      : peopleError ?? "구성원 데이터를 불러올 수 없습니다."}
                  </p>
                )
              ) : null}
              {tab === "members" ? (
