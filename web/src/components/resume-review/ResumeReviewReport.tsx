...
import { ArrowRight, CheckCircle2, Lightbulb, Quote, Target } from "lucide-react";
...
export function ResumeReviewReport({
  ...
  const displayScore = jdMatch.matchScore ?? 0;
  const hasScore = jdMatch.matchScore != null;
    ...
    <div className="space-y-8">
        ...
        <div className="flex w-full flex-col items-center gap-3 sm:w-auto">
          ...
          {hasScore ? (
            <ScoreGauge
              value={displayScore}
              label="키워드 매칭"
              variant={matchScoreVariant(jdMatch.matchScore)}
            />
          ) : (
            <div className="flex h-28 w-28 flex-col items-center justify-center rounded-full border-2 border-dashed border-card-border text-center">
              <span className="text-xs text-muted">매칭률</span>
          ...
          {hasScore && (
            <span className="rounded-full bg-gold/15 px-3 py-0.5 text-xs font-medium text-gold">
              {matchScoreLabel(jdMatch.matchScore)}
            </span>
          )}
        </div>
          ...
          {jdMatch.matched.length > 0 && (
            <section className="card-luxe p-5">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="font-semibold text-foreground">잘 드러난 키워드</h3>
              </div>
          ...
          {jdMatch.missing.length > 0 && (
            <section className="card-luxe p-5">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="font-semibold text-foreground">보완이 필요한 키워드</h3>
              </div>
              <div className="flex flex-wrap gap-2">
      ...
      {paragraphFeedback.length > 0 && (
        <section className="card-luxe space-y-5 p-6">
          <div>
            <h2 className="text-lg font-semibold text-foreground">문단별 첨삭</h2>
            <p className="mt-1 text-sm text-muted">
              원문 인용을 바탕으로 지적 사항과 수정 방향을 정리했습니다.
            </p>
          </div>
                ...
                <div className="space-y-4 p-4 text-sm">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-lg bg-warning/8 p-3">
                      <p className="text-xs font-semibold text-warning">지적</p>
                      <p className="mt-1.5 leading-relaxed text-foreground">{p.issue}</p>
                    </div>
                    <div className="rounded-lg bg-accent/8 p-3">
                      <p className="text-xs font-semibold text-accent">제안</p>
      ...
      {improvementPlan.length > 0 && (
        <section className="card-luxe space-y-4 p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-foreground">우선 보완 액션</h2>
          </div>
