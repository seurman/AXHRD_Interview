...
## 최근 작업 — 메뉴에 학습 패스·인증서 반영 (진행 중)

| 항목 | 내용 |
|------|------|
| 게스트 제품 | 자소서·역량 학습 패스·역량 트래킹 진입 추가 (가입 next) |
| 연습 메뉴 | 패스 → 질문카드 → 체험 순으로 정렬 |
| 계정 | 아바타·모바일 더보기에 「역량 인증서」 |

## 최근 작업 — 개인 대시보드 안정화 (진행 중)

| 항목 | 내용 |
|------|------|
| 격리 | 학습 패스/약점 조회 실패 시에도 대시보드 본문 렌더 |
| 시드 | 대시보드 읽기 경로에서 레슨 카탈로그 auto-seed 제거 |
| 레이아웃 | `max-lg:order-first` · `min-w-0` · 약점 질문 truncate · product-stage overflow 완화 |
| 스킬바 | IRT 레벨 유지 + 패스 인증 병기, 역량 고정 순서 |

## 최근 작업 — 학습 패스 모바일 최적화 (진행 중)

| 항목 | 내용 |
|------|------|
| 터치 | 패스/약점 CTA·트랙 토글·퀴즈 선택·공유 버튼 `min-h-11` |
| 대시보드 | 모바일에서 `LearningPathCard`를 차트보다 먼저 표시 |
| 홈 | ≤640px 제품 그리드 1열 · CTA 세로 스택 · 푸터 링크 터치 영역 |
| 인증서 | 패딩·뱃지·역량 행 모바일 스택 |

## 최근 작업 — 약점 드릴·인증서·홈 반영 (2026-07-21)
