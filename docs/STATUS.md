...
| 항목 | 내용 |
|------|------|
| 코드 | 의도 독해·베스트/워스트 포함 — 배포 시 플레이어 패스에 기본 활성 |
| DB | `CompetencyGameRuntimeConfig` — 타입/레벨 비활성 목록 |
| 관리 | `/admin/content/game` — 타입 킬스위치 + 레벨별 넣고 빼기 |
| 런타임 | `getEnabledGameCourse`가 패스·해금·complete API에 반영 |
...
| 항목 | 내용 |
|------|------|
| 타입 | `intent_read` · `best_worst` (지문 독해형, SJT 직후 배치) |
| UX | 지문→의도 MCQ · 장면→베스트+워스트 이중 선택 |
| 뱅크 | `reading-banks.ts` 6역량 × 각 3문항 · 단어장 신호어 explain 연결 |
| 해금 | 중간 삽입 시 이후 클리어 사용자는 백필 해금 |
